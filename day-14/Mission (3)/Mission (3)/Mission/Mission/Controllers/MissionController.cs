...
namespace Mission.Controllers
{
    ...
    public class MissionController(IMissionService missionService) : ControllerBase
    {
        ...
        public async Task<IActionResult> MissionApplicationDelete(int id)
        {
            try
            {
                var result = await _missionService.MissionApplicationDelete(id);
                if (!result)
                {
                    return NotFound(new ResponseResult() { Data = null, Result = ResponseStatus.Error, Message = "Application not found." });
                }

                return Ok(new ResponseResult() { Data = result, Result = ResponseStatus.Success, Message = "Application deleted successfully." });
            }
            catch (Exception ex)
            {
                return BadRequest(new ResponseResult() { Data = null, Message = ex.Message, Result = ResponseStatus.Error });
